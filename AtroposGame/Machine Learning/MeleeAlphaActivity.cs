...
//        where T : struct
//    {
//        protected static MeleeAlphaActivity<T> Current { get { return (MeleeAlphaActivity<T>)CurrentActivity; } set { CurrentActivity = value; } }
...
//            SetContentView(Resource.Layout.MeleeAlpha);
//            FindAllViews();
...
//        protected void GiveCueAndWait(GestureClass targetClass)
//        {
//            Task.Run(async () =>
//            {
//                var initialDelay = (Res.Random * 5 + 1) * (Res.Random * 4 + 1);
//                await Task.Delay((int)initialDelay);

//                ResetStage($"Cueing {targetClass?.className}");

//                Speech.Say(targetClass?.className ?? "Anything");

//                // Now start the gesture recognition stage and run until it comes back saying it's got one.
//                var resultSeq = await ((CuedGestureRecognizerStage)CurrentStage).RunUntilFound(targetClass);
