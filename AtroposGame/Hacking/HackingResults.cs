...
namespace Atropos.Hacking
{
    ...
    //    public class HackTaskDefinition : GameEffectDefinition
    //    {
    //        public const string HackPrefix = "Hacking.";
    //        public string TaskName;
    //        public override string Name { get => HackPrefix + TaskName; }

    //        public virtual string OnSelectionString { get; protected set; }
    //        public virtual double OnSelectionStringSpeechRate { get { return 1.0 + (OnSelectionString?.Length ?? 0) / 25; } }
    ...
    //        public static event EventHandler<EventArgs<object>> UponSelection;
    //        protected Func<object, Task> onSelection;
    //        public async void OnSelection(object data = null)
    //        {
    //            UponSelection.Raise(data);
    //            await OnGenerate(data);
    //        }
    //        public override async Task OnGenerate(object data = null)
    //        {
    //            if (!String.IsNullOrEmpty(OnSelectionString)) Speech.Say(OnSelectionString, null, true, null, 1.0, OnSelectionStringSpeechRate);
    //            Log.Debug(_tag, $"Selected {TaskName}.");
    //            await onSelection?.Invoke(data);
    //        }
    ...
    //            recipientSoundOptions.UseSpeakers = false; // No need for us both to put it up on speakers, but target might not be in a position to hear the speakers from the caster.
    //            CastSFX.Play(recipientSoundOptions);
    //            Log.Debug(_tag, $"Being affected by {TaskName}.");
    ...
    //            else onHitBy.Invoke(null, $"{Hacker.EFFICACY}:{instance[Hacker.EFFICACY]}");
    //        }

    //        public async Task SendDamage(DamageType type, double baseMagnitude, double toBarrier = 1.0)
    //        {
    //            var damage = new Damage() { Type = type, Magnitude = baseMagnitude * Res.GetRandomCoefficient(0.25) * Hacker.Me.HackingEfficacy, ToBarriers = toBarrier };
    ...
    //            if (onHitBy == null)
    //            {
    //                Log.Warn(_tag, $"Attempted to send the OnHitBy effect of {TaskName}... which doesn't have one... to {target.Name}.");
    //                return;
    //            }
    //            if (target == Communications.CommsContact.Nobody && !Res.SolipsismMode)
    //            {
    //                Log.Warn(_tag, $"Attempted to send the OnHitBy effect of {TaskName} to... nobody.  Okay... done.");
    //                return;
    //            }
    ...
    //        public HackTaskDefinition(string name, params Glyph[] glyphs) : base()
    //        {
    //            TaskName = name;
    ...
    //        public static HackTaskDefinition Dart = new HackTaskDefinition("Dart", Glyph.L, Glyph.A, Glyph.P)
    //        {
    //            CastSFXresourceID = Resource.Raw.fzazzle_magic,
    //            CastSFXname = "Magic.Fzazzle",
    //            onSelection = async (o) => await Dart.SendDamage(DamageType.Piercing, 20),
    //            onHitBy = async (caster, o) => await SufferDamage(o)
    //        };

    //        public static HackTaskDefinition Zap = new HackTaskDefinition("Zap", Glyph.L, Glyph.A, Glyph.Z, Glyph.P)
    //        {
    //            CastSFXresourceID = Resource.Raw._136542_electricZap_small,
    //            CastSFXname = "Magic.ElectricZapSmall",
    //            Keywords = Listify("Electricity"),
    //            onSelection = async (o) => await Zap.SendDamage(DamageType.Lightning, 25, 0.75),
    ...
    //                var dazeKey = Hacker.Me.AddDaze(1.1);
    //                await Task.Delay(10000);
    //                Hacker.Me.RemoveDaze(dazeKey);
    //            }
    //        };
    ...
    //        public static HackTaskDefinition Barrier = new HackTaskDefinition("Barrier", Glyph.L, Glyph.D, Glyph.S)
    //        {
    ...
    //                var barrierAmount = BarrierBaseMagnitude * Hacker.Me.HackingEfficacy * y;
    ...
    //        public static HackTaskDefinition Shield = new HackTaskDefinition("Shield", Glyph.M, Glyph.D, Glyph.X, Glyph.P)
    //        {
    ...
    //        public static HackTaskDefinition Lance = new HackTaskDefinition("Lance", Glyph.M, Glyph.A, Glyph.R, Glyph.P)
    //        {
    //            CastSFXresourceID = Resource.Raw._366093_kwongg,
    //            CastSFXname = "Magic.DenseMetallicBoom",
    //            onSelection = async (o) => await Lance.SendDamage(DamageType.Impact, 50),
    //            onHitBy = async (caster, o) => await SufferDamage(o)
    //        };
    ...
    //        public static HackTaskDefinition Frost = new HackTaskDefinition("Frost", Glyph.M, Glyph.A, Glyph.I, Glyph.P)
    //        {
    //            CastSFXresourceID = Resource.Raw._346916_fwissh,
    //            CastSFXname = "Magic.Fwissh",
    //            onSelection = async (o) => await Frost.SendEffect(data: $"{Hacker.EFFICACY}:{Hacker.Me.HackingEfficacy:f2}"),
    //            onHitBy = async (caster, o) =>
    //            {
    //                if (Damageable.Me.Shielded)
    //                {
    ...
    //                if (!(o is string oStr) || !oStr.StartsWith(Hacker.EFFICACY)) throw new ArgumentException();
    //                var efficacy = double.Parse(oStr.Split(':', ';')[1]);
    ...
    //        public static HackTaskDefinition Flame = new HackTaskDefinition("Flame", Glyph.H, Glyph.A, Glyph.F, Glyph.P)
    //        {
    //            CastSFXresourceID = Resource.Raw._248116_fwehshh,
    //            CastSFXname = "Magic.Fwehshh",
    //            onSelection = async (o) => await Flame.SendEffect(data: $"{Hacker.EFFICACY}:{Hacker.Me.HackingEfficacy:f2}"),
    //            onHitBy = async (caster, o) =>
    //            {
    //                if (Damageable.Me.Shielded)
    //                {
    ...
    //        public static HackTaskDefinition Clarity = new HackTaskDefinition("Clarity", Glyph.L, Glyph.C, Glyph.X, Glyph.K)
    //        {
    //            CastSFXresourceID = Resource.Raw.tinkly_magic,
    //            CastSFXname = "Magic.Tinkly",
    //            onSelection = async (o) =>
    //            {
    //                foreach (var key in Hacker.Me.EaseMultipliers.Keys)
    //                    Hacker.Me.RemoveDaze(key);
    //                Speech.Say("All daze effects removed.  Clarity of thought returns.");
    //                Hacker.Me.AddDaze("Clarity", 1.0 - 0.2 * Hacker.Me.HackingEfficacy);
    ...
    //                await Task.Delay(20000 + (int)(25000 * Hacker.Me.HackingEfficacy));
    //                claritySound.Stop();
    //                Hacker.Me.RemoveDaze("Clarity");
    //            }
    //        };

    //        public static HackTaskDefinition Dazzle = new HackTaskDefinition("Dazzle", Glyph.M, Glyph.C, Glyph.X, Glyph.P)
    //        {
    //            CastSFXresourceID = Resource.Raw._234804_transmat,
    //            CastSFXname = "Magic.Transmat",
    //            onSelection = async (o) => await Dazzle.SendEffect(data: $"{Hacker.EFFICACY}:{Hacker.Me.HackingEfficacy:f2}"),
    //            onHitBy = async (caster, o) =>
    //            {
    ...
    //                var dazeKey = Hacker.Me.AddDaze(1.0 + 0.2 * efficacy);
    //                await Task.Delay(5000 + (int)(5000 * efficacy));
    //                Hacker.Me.RemoveDaze(dazeKey);
    //            }
    //        };

    //        public static HackTaskDefinition Paralyze = new HackTaskDefinition("Paralyze", Glyph.H, Glyph.C, Glyph.N, Glyph.I, Glyph.B)
    //        {
    //            CastSFXresourceID = Resource.Raw._366092_zoom_slowing_down,
    //            CastSFXname = "Magic.ZoomSlowingDown",
    //            onSelection = async (o) => await Paralyze.SendEffect(data: $"{Hacker.EFFICACY}:{Hacker.Me.HackingEfficacy:f2}"),
    //            onHitBy = async (caster, o) =>
    //            {
    //                if (Damageable.Me.Shielded)
    //                {
    ...
    //                Hacker.Me.AddDaze("Paralysis", 100.0);
    //                var movedHowFast = new ConditionStage<float>(stillnessProvider,
    //                    (f, t) =>
    //                    {
    ...
    //                    });
    //                await movedHowFast.ConditionMet();
    //                Hacker.Me.RemoveDaze("Paralysis");
    //                paralysisSound.Stop();
    //            }
    //        };

    //        public static List<HackTaskDefinition> AllSpells
    //        {
    //            get => GameEffect
    //                    .AllDefinitions
    //                    .Where(geff => geff.Name.StartsWith(HackPrefix))
    //                    .Select(geff => geff as HackTaskDefinition)
    //                    .ToList();
    //        }
    //    }
}